...
function BurdJournals.Server.handleSaveDebugJournalBackup(player, args)
    ...
    local normalizedJournalData = BurdJournals.normalizeJournalData(journalData) or journalData

    -- Store to server-side global ModData
    local cache = BurdJournals.Server.getDebugJournalCache()

    -- Deep copy the data to ensure clean storage
    cache.journals[journalKey] = {
        ...
        stats = {},
        skillReadCounts = {},
        drLegacyMode3Migrated = normalizedJournalData.drLegacyMode3Migrated == true,
        migrationSchemaVersion = tonumber(normalizedJournalData.migrationSchemaVersion) or 0,
        isDebugSpawned = true,
        isDebugEdited = normalizedJournalData.isDebugEdited or true,
        isPlayerCreated = normalizedJournalData.isPlayerCreated or true,
        sanitizedVersion = normalizedJournalData.sanitizedVersion,
        uuid = normalizedJournalData.uuid,
        itemType = normalizedJournalData.itemType,
        itemID = normalizedJournalData.itemID,
        readCount = tonumber(normalizedJournalData.readCount) or 0,
        readSessionCount = tonumber(normalizedJournalData.readSessionCount) or 0,
        currentSessionId = normalizedJournalData.currentSessionId,
        currentSessionReadCount = tonumber(normalizedJournalData.currentSessionReadCount) or 0,
        timestamp = getTimestampMs and getTimestampMs() or os.time(),
        savedBy = player:getUsername(),
    }

    -- Copy skills
    if normalizedJournalData.skills then
        for skillName, skillData in pairs(normalizedJournalData.skills) do
    ...
    -- Copy traits
    if normalizedJournalData.traits then
        for traitId, value in pairs(normalizedJournalData.traits) do
            if traitId then
                cache.journals[journalKey].traits[traitId] = value
            end
        end
    end

    -- Copy recipes
    if normalizedJournalData.recipes then
        for recipeName, value in pairs(normalizedJournalData.recipes) do
            if recipeName then
                cache.journals[journalKey].recipes[recipeName] = value
            end
        end
    end

    -- Copy stats
    if normalizedJournalData.stats then
        for statId, statData in pairs(normalizedJournalData.stats) do
            if statId then
                cache.journals[journalKey].stats[statId] = statData
            end
        end
    end

    -- Copy per-skill diminishing counters.
    local skillReadCounts = normalizedJournalData.skillReadCounts
...
function BurdJournals.Server.handleDebugRecalcBaseline(player, args)
    ...
    if not targetPlayer then
        BurdJournals.Server.sendToClient(player, "debugError", {message = "Target player not found"})
        return
    end
    ...
    modData.BurdJournals_Baseline = modData.BurdJournals_Baseline or {skills = {}, traits = {}, recipes = {}}
    modData.BurdJournals_Baseline.skills = baselineData.skillBaseline or {}
    modData.BurdJournals_Baseline.traits = baselineData.traitBaseline or {}
    modData.BurdJournals_Baseline.recipes = baselineData.recipeBaseline or {}
